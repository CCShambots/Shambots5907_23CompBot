...
public class ArmTrajectory {
  ...
  private final Map<Integer, ArmState> states =
      new HashMap<>(); // List of states at various steps along the trajectory (in 20 ms intervals)

  public ArmTrajectory(
      ArmKinematics kinematics, ArmState currentArmState, Pose3d endPose, double time) {
    ...
    ArmState lastState =
        kinematics.chooseIdealState(currentArmState, kinematics.inverseKinematics(startPose));

    for (int i = 0; i < steps; i++) {
      currentPose = currentPose.transformBy(perStepTransform);
      ArmState currentState =
          kinematics.chooseIdealState(lastState, kinematics.inverseKinematics(currentPose));
