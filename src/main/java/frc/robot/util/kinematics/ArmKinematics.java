...
public class ArmKinematics {
  ...
  public ArmKinematics(
      double baseToTurret, double elevatorToArm, double armToWrist, double wristToEndEffector) {
  ...
  public Pose3d forwardKinematics(ArmState state) {
    ...
    // Turret pose
    workingPose =
        workingPose.transformBy(
            new Transform3d(
                new Translation3d(0, 0, baseToTurret),
                new Rotation3d(0, 0, state.getTurretAngle())));

    // Shoulder pose
    workingPose =
        workingPose.transformBy(
            new Transform3d(
                new Translation3d(0, 0, turretToArm + state.getElevatorExtension()),
    ...
    workingPose =
        workingPose.transformBy(
            new Transform3d(
                new Translation3d(armToWrist, new Rotation3d(0, -state.getShoulderAngle(), 0)),
                new Rotation3d(0, -state.getShoulderAngle(), 0)));
    ...
    workingPose =
        workingPose.transformBy(
            new Transform3d(
                new Translation3d(
                    wristToEndEffector, new Rotation3d(0, -(state.getWristAngle()), 0)),
    ...
    workingPose =
        workingPose.transformBy(
            new Transform3d(
                new Translation3d(0, 0, 0), new Rotation3d(-state.getRotatorAngle(), 0, 0)));
  ...
  public ArmState[] inverseKinematics(Pose3d targetPose) {
    ...
    // Get turret angle
    // This could be done from the yaw of the pose, but I think that's dumb because they shouldn't
    // have to calculate turret angle manually
    ...
    Pose2d pose2d2 =
        pose2d.transformBy(
            new Transform2d(new Translation2d(-w, new Rotation2d(0)), new Rotation2d(0, 0)));

    // Determine the height of the triangle formed by arm joint, wrist, joint, and intersection of
    // those two to the elevator.
  ...
  public ArmState chooseIdealState(ArmState currentState, ArmState... options) {
    ...
    for (int i = 0; i < options.length; i++) {
      ...
      double currentEDistance =
          abs(currentState.getElevatorExtension() - test.getElevatorExtension());
